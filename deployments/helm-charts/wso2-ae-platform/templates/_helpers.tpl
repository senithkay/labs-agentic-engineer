{{/*
Sanitise an image tag into a DNS-label-safe suffix for immutable resource names.
"latest" → "latest"
"1.2.3"  → "1-2-3"
Truncated to 40 chars so the full resource name stays under 63 chars.
*/}}
{{- define "ae-platform.tagSuffix" -}}
{{- . | lower | replace "." "-" | replace ":" "-" | replace "/" "-" | trunc 40 | trimSuffix "-" -}}
{{- end }}

{{/*
Full spec snapshot for the platform-service ClusterComponentType.
Embedded verbatim in each ComponentRelease that uses this type.
OC requires the complete spec snapshot in the immutable ComponentRelease.
*/}}
{{- define "ae-platform.platformServiceSpec" -}}
  ...
  openAPIV3Schema:
    ...
    properties:
      serviceAccountName:
        type: string
        default: ""
        description: "K8s ServiceAccount to run the pod under. Empty = default SA."
environmentConfigs:
  openAPIV3Schema:
    type: object
    properties:
    ...
    template:
      ...
      spec:
        ...
        template:
          ...
          spec:
            serviceAccountName: "${parameters.serviceAccountName != '' ? parameters.serviceAccountName : oc_omit()}"
            containers:
              - name: main
