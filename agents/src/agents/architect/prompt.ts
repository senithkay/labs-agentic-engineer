...
# Rules for components
  ...
  - **Authentication is delegated to the platform IDP — DO NOT introduce a separate auth / identity / login / session component, and DO NOT implement \`/auth/login\` or \`/auth/register\` in any service.** When the spec implies users sign in:
      ...
      * Set \`auth: { kind: "oidc-spa", upstream: <api-name> }\` on the web-app that signs the user in. The platform posts a \`## OIDC client provisioned\` comment on the SPA's task issue with four values (\`issuer\`, \`clientId\`, \`scopes\`, \`host\`). The agent bakes these into \`workload.yaml\`'s \`configurations.env\` block as \`OIDC_ISSUER\`, \`OIDC_CLIENT_ID\`, \`OIDC_SCOPES\`, \`OIDC_HOST\`. nginx envsubst renders them into \`/env-config.js\` + a same-origin \`/oidc/\` proxy at pod start; the SPA reads \`window.__ENV__\` at runtime.
      ...
      * The web-app's \`componentAgentInstructions\` MUST say: "OIDC Authorization Code + PKCE against the platform IDP. Read OIDC config from \`window.__ENV__\` (rendered by nginx envsubst — do NOT bake values into the bundle). Token exchange MUST go through \`window.__ENV__.OIDC_TOKEN_URL\` (a relative \`/oidc/token\` — the SPA's own nginx proxies it same-origin to Thunder's \`/oauth2/token\`, bypassing a kgateway CORS bug). The authorize redirect uses absolute \`window.__ENV__.OIDC_ISSUER\` (top-level navigation — no CORS). Attach \`Authorization: Bearer <access_token>\` to every \`window.__ENV__.API_BASE_URL\` call. The redirect URI is \`window.location.origin + '/callback'\`. The Workload's \`configurations.env\` MUST declare \`OIDC_ISSUER\`, \`OIDC_CLIENT_ID\`, \`OIDC_SCOPES\`, \`OIDC_HOST\`, \`API_BASE_URL\` — values come from the issue's \`## OIDC client provisioned\` and \`## Dependency endpoint resolved\` comments. See the \`asdlc\` SKILL's OIDC-SPA section for the nginx template + reference PKCE snippet." NEVER write a \`/login\` form that POSTs credentials to the API.
