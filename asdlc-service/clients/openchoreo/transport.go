...
//
// Returns empty string when no provider is configured (local k3d, OC accepts
// unauthenticated calls in the OSS default).
func authToken(_ context.Context, ap AuthProvider) string {
	if ap == nil {
		return ""
	}
	tok, err := ap.Token()
	if err != nil {
		slog.Error("openchoreo: service token fetch failed", "error", err)
		return ""
	}
	return tok
}
