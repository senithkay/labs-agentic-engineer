...
| Local freshness (org repo → dev clone) | developer machine | Go — new `aep skills sync` command in the existing `tools/aectl` CLI (binary `aep`, already has `login` + `~/.aep/config.yaml`); writes the project skill copies + `CLAUDE.md` "load these first" note | manual, **optional** | convenience — ship last |
