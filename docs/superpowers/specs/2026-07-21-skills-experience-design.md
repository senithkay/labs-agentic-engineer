...
| Local freshness (org repo → dev clone) | developer machine | Go — new `aep skills sync` command in the existing `tools/aectl` CLI (binary `aep`, already has `login` + ConfigMap-backed config via `aep-cli-config`; config seeded with `aep platform config import`); writes the project skill copies + `CLAUDE.md` "load these first" note | manual, **optional** | convenience — ship last |
